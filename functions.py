...
def plot_subplots_ensemble_members_and_mean(models, model_times_by_model, model_nao_anoms_by_model, obs_nao_anom, obs_time):
    ...
    for i, model_name in enumerate(models):
        ...
        ax.plot(model_time, ensemble_mean, color="red", label=f"{model_name} (ACC: {acc_score:.2f}, RPC: {rpc:.2f})")

        # Plot the 5-95% confidence intervals
        ax.fill_between(model_time, conf_interval_lower, conf_interval_upper, color="red", alpha=0.2, label="5-95% confidence interval")
